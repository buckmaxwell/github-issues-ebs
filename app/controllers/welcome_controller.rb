class WelcomeController < ApplicationController
  ...
  def get_combined_collab_history_chart(collabs)
    result = [%w[Estimate Perfect]] #=> [['Estimate', 'Perfect', 'CollabName1', 'CollabName2', etc.], [.....]]
  ...
  def get_repos
    ...
    while (repos.length < 500) && continue
  ...
  def get_data_store(milestone)
    ...
    if @selected_priority == 'all'
      ...
      issues_for_label = paginate(issues_for_label, 100, 10_000)
    ...
    100.times do |_x|
      ...
      issues.each do |issue|
        sfs[issue.assignee.login] = [] if sfs[issue.assignee.login].nil?
        ...
        sfs.delete(issue.assignee.login) if sfs[issue.assignee.login].empty?
      end
  ...
  def get_past_velocities_and_history_tuples
    ...
    issues.each do |issue|
      ...
      next if v.nil? || t.nil?
      if result1[issue.assignee.login].nil? && result2[issue.assignee.login].nil?
  ...
  def paginate(page1_result, per_page, max)
    ...
    while (result.length < max) && continue
      # new_data = @client.last_response.rels[:next].get.data
  ...
  def get_estimate(issue)
    unless issue.body.blank? || issue.assignee.nil? || Collaborator.find_by_login(issue.assignee.login).nil?
      ...
      initial_est.to_f / collab.random_velocity if is_numeric? initial_est
    end
  end
  ...
  def get_estimate_and_actual(issue)
    unless issue.body.blank?
      ...
      puts est
      puts act
      [est.to_f, act.to_f] if is_numeric?(est) && is_numeric?(act)
    end
  end
